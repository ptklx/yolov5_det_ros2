...
def main_camera_pcl():
    ...
    try:
        while True:
            ...
            ###########
            det_pred=qnn_yolo(color_image)
            circle_frame=color_image
            if len(det_pred):
                res_img = draw_detect_res(color_image, det_pred)

                # cv2.imshow('Output', cv2.resize(res_img, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_LINEAR))
                # cv2.waitKey(0)
                # one_point = left2right_point(det_pred,need_class=0)   # x,y  0 person
                det_pred_tmp = customized_filtration_min_max(color_image, det_pred)  
                one_point = middleorleft2right_point(det_pred_tmp,(color_image.shape[0],color_image.shape[1]),need_class=1)
                if one_point==[]:
                    continue
                dist = get_mid_pos(one_point,depth_image,24)
                print(f"distance:{dist}")
                circle_frame = draw_circle_res(res_img,one_point,dist)
                print(one_point)
            ...
            images = np.hstack((circle_frame, depth_colormap))
            # Show images
...
def main_camera():
    from cut_yolov5_soda_beer_onnx import yolov5det as yolov5_soda_beer
    ...
    path_onnx = os.path.join(os.path.dirname(os.path.abspath(__file__)),"models/cutoff_soda_beer_water.onnx")
    model_so_b = yolov5_soda_beer(path_onnx)
    ...
    time_st = 30  ###
    st =0
    result=[4,0,0,0,0]  #soda=0  beer=1  water = 2 coke=3   fanta=4
    while True:
        ret, frame = cap.read()
        circle_frame=frame
        # 如果成功读取帧
        ...
        if result[0] != -1 and st==0:
            need_class = result[0]-1  if  result[0]>0  else result[0]
            print("start detect !")
            ...
            if len(preds):
                ...
                if need_class==0:
                    det_pred_soda=model_so_b(frame)  
