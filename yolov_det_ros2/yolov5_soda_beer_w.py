...
coco_class = [
     'water', 'soda', 'beer', 'airplane', 'bus', 'train', 'truck', 'boat', 'traffic light',
...
class yolov5det:
    ...
    def init_qnn(self):
        ...
        self.output_shapes = [[1, 20, 20, 24], [1, 40, 40, 24], [1, 80, 80, 24]]
...
def main():
    ...
    qnn_yolo = yolov5det(os.path.join(os.path.dirname(os.path.abspath(__file__)),'models/cutoff_8550_soda_beer_water_fp16.qnn216.ctx.bin'))
    # image process
    ...
    print("=======================================")
    if len(det_pred):
        # det_pred = customized_filtration(frame, preds)   # 删除不在范围中的
        # one_point = left2right_point(det_pred,need_class=result[1])  # x,y
        one_point = left2right_point(det_pred,need_class=0)  # x,y
        if one_point==[]:
